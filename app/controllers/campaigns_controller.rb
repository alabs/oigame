...
class CampaignsController < ApplicationController
  ...
  private
    ...
    def generate_stats_for_petition(campaign)
      ...
      petitions = 0
      dates.each do |date|
        count = Rails.cache.fetch("stats_for_petition_#{campaign.id}_#{date.to_s}", :expires_in => 3.hour) { Petition.validated.where("created_at BETWEEN ? AND ?", date, date.tomorrow.to_date).where(:campaign_id => campaign.id).all }.count
        petitions += count
